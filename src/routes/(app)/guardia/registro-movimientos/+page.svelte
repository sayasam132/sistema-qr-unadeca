		...
		<div class="registro__tabla">
			...
			{#each movimientosFiltrados as movimiento (movimiento.id)}
				<div class="registro__fila">
					<span data-etiqueta="Nombre"
						>{movimiento.usuarios?.nombre} {movimiento.usuarios?.apellido}</span
					>
					<span data-etiqueta="Identificación"
						>{movimiento.usuarios?.identificacion || movimiento.usuarios?.carnet || '—'}</span
					>
					<span
						data-etiqueta="Tipo"
					...
					<span data-etiqueta="Fecha">{formatearFecha(movimiento.created_at)}</span>
					<span data-etiqueta="Hora">{formatearHora(movimiento.created_at)}</span>
				</div>
			{/each}
		</div>
	{/if}
