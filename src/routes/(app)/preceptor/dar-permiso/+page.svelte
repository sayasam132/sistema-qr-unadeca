<script lang="ts">
	...
	async function resolver(permiso: (typeof data.permisos)[number], accion: 'aprobar' | 'rechazar') {
		error = null;
		...
		try {
			...
			} else if (permiso.tipo_permiso === 'fin_de_semana') {
				payload = { aprobado_preceptor: true };
			} else {
			...
			// El fin de semana necesita también la aprobación de admisión; el
			// trigger de la base de datos calcula el estado final y solo se
			// notifica al estudiante cuando ya quedó resuelto.
			if (actualizado.estado !== 'pendiente') {
				...
				const { error: errorNotificacion } = await supabase.from('notificaciones').insert({
					...
					mensaje: `Tu solicitud de ${etiquetaTipo} del ${formatearFecha(permiso.fecha_salida)} fue ${actualizado.estado} por tu preceptor/a.`,
...
<PanelPagina
	titulo="Dar permiso"
	subtitulo="Salidas del día y fin de semana de tu hogar"
	{iniciales}
>
	{#if error}
