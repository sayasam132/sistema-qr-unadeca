...
export const load: PageServerLoad = async ({ locals: { supabase } }) => {
	const { data: preceptores } = await supabase
		.from('usuarios')
		.select('id, nombre, apellido, genero')
		.eq('tipo_usuario', 'preceptor')
		.order('nombre', { ascending: true });

	return { preceptores: preceptores ?? [] };
};
...
export const actions: Actions = {
	crear: async ({ request }) => {
		...
		const { data: alta, error: errorAlta } = await supabaseAdmin.auth.admin.createUser({
			...
			password: contrasena,
			email_confirm: true,
			user_metadata: { nombre, apellido, tipo_usuario: 'preceptor' }
		});
		...
		// en este formulario: el preceptor la completa desde Mi Perfil la
		// primera vez que entra, y ahí queda bloqueada. Se usa el correo como
		// placeholder para que nunca choque con el de otra cuenta.
	...
	eliminar: async ({ request }) => {
		...
		if (!id) {
			return fail(400, { error: 'Falta el id del preceptor a eliminar.' });
		}
		...
		if (usuario?.tipo_usuario !== 'preceptor') {
			return fail(400, { error: 'Esa cuenta no es de un preceptor.' });
		}
