...
<svelte:head>
	<title>Crear cuenta Guardia · UNADECA</title>
</svelte:head>

<PanelPagina titulo="🛡️ Crear cuenta Guardia" subtitulo="Alta de personal de seguridad" {iniciales}>
	{#if form?.exito}
	...
	<div class="cuenta__columnas">
		...
		<div class="cuenta__lista">
			<p class="cuenta__lista-titulo">Lista de Guardias</p>
			{#if data.guardias.length === 0}
				<p class="cuenta__lista-vacio">Todavía no hay guardias registrados.</p>
			{:else}
				{#each data.guardias as guardia (guardia.id)}
					<form
						method="POST"
						action="?/eliminar"
						use:enhance={({ cancel }) => {
							const nombreCompleto = `${guardia.nombre} ${guardia.apellido}`;
							if (
								!confirm(
					...
					>
						...
						<div class="cuenta__chip" class:cuenta__chip--femenino={guardia.genero === 'femenino'}>
							<span>{guardia.nombre} {guardia.apellido}</span>
							<button
								type="submit"
								class="cuenta__chip-borrar"
								disabled={eliminandoId === guardia.id}
								aria-label={`Eliminar a ${guardia.nombre} ${guardia.apellido}`}
							>
								×
							</button>
						</div>
